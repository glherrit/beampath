...
export function calcSag(radius: number, R: number, k: number): number {
	const sag =
		(radius * radius) / R / (1 + Math.sqrt(1 - ((1 + k) * Math.pow(radius, 2)) / Math.pow(R, 2)));
	return sag;
}

export function genLensLathe(
	ap: number,
	Radius1: number,
	Radius2: number,
	ct: number,
...
export function toGrid(zpoint: number, scale: number[][]): number {
	...
	return (
		((zpoint - scale[0][0]) * (scale[1][1] - scale[1][0])) / (scale[0][1] - scale[0][0]) +
		scale[1][0]
	);
}
...
export function toWorld(gpoint: number, scale: number[][]): number {
	...
	return (
		((gpoint - scale[1][0]) * (scale[0][1] - scale[0][0])) / (scale[1][1] - scale[1][0]) +
		scale[0][0]
	);
}
...
export function points2ArrayX(
	...
	const zeroLinesegs = new Float32Array(y.length * 3);
	for (let i = 0; i < numPoints; i++) {
		...
		zeroLinesegs[i * 3] = x;
		zeroLinesegs[i * 3 + 1] = 0;
		zeroLinesegs[i * 3 + 2] = z[i];
	}
	return [pluslinesegs, zeroLinesegs, neglinesegs];
}
